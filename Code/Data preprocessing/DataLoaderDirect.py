...
class DataLoader:
    ...
    normalImagesTrainPath = os.path.join(root, 'lines.h5')
    labelsTrainPath = os.path.join(root, 'labels_lines.h5')
    ...
    deslantedImagesTrainPath = os.path.join(root, 'deslant_lines.h5')
    ...
    def __init__(self, batchSize, imageSize, maxTextLength,deslant,augment = False, rotate = False):
        ...
        self.validationImages = hf.get(valid1[valid1.rfind('/') + 1:valid1.rfind('.')]).value
        ...
        self.validationLabels = np.array(f[list(f.keys())[0]])
