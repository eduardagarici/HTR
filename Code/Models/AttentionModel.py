...
class Model:
    ...
    def setupRNN(self, rnnIn4d):
        ...
        encoder, forward_h, forward_c, backward_h, backward_c = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(numHidden, return_sequences= True, return_state=True), merge_mode="concat")(rnnIn3d)

        state_h = tf.concat([forward_h, backward_h], axis = 1)
        state_c = tf.concat([forward_c, backward_c], axis = 1)

        print("FORWARD_H:", forward_h)
        print("STATE_H:", state_h.shape)
        print("STATE_C:", state_c.shape)

        attention_mechanism = tf.contrib.seq2seq.BahdanauAttention(num_units = numHidden, memory = encoder)
        cell = tf.nn.rnn_cell.BasicLSTMCell(numHidden)
        decoder = tf.contrib.seq2seq.AttentionWrapper(cell,attention_mechanism = attention_mechanism, attention_layer_size=512)

        outputs = []
        new_state = decoder.zero_state(self.batchSize, tf.float32)
        for i in range(encoder.shape[1]):
            output,new_state = decoder.apply(encoder[:,i,:], new_state)
            outputs.append(output)
        decoder = tf.stack(outputs , axis = 1)
        decoder = tf.expand_dims(decoder, axis = 2)

        kernel = tf.Variable(tf.truncated_normal(
            [1, 1, numHidden, len(self.charList) + 1], stddev=0.1))

        return tf.squeeze(tf.nn.atrous_conv2d(value=decoder, filters=kernel, rate=1,
                                              padding='SAME'),axis=2)

    def setupCTC(self, ctcIn3d):

        ctcIn3dTBC = tf.transpose(ctcIn3d, [1, 0, 2])
        print("CTC:",ctcIn3dTBC.shape)
    ...
    def setupTF(self):
        ...
        modelDir = 'drive/My Drive/Licenta/Cod/Checkpoints/IAM/Attention/'
    ...
    def save(self):
        """ Save model to file """
        self.snapID += 1
        self.saver.save(self.sess, 'drive/My Drive/Licenta/Cod/Checkpoints/IAM/Attention/',
